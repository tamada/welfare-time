...
大学の食堂（Cafeteria）やキッチンカー（Kitchen Car）のスケジュール情報を自動収集し、APIとして公開・ホスティングするプロジェクトです。
PDFやWebサイトなど情報が散らばっているので、これらを統合して提供することを目的としています。

## 概要

このプロジェクトは、以下の2つの主要なデータソースから情報を取得し、正規化して静的なJSON APIとして配信します。

1. **学食スケジュール**: PDF形式のメニュー表から情報を抽出
2. **キッチンカー**: Webサイトからスケジュール情報をスクレイピング
